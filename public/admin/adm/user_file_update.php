...
$sql = "select * from {$g5['member_table']} where id = '$u_id'";
$row = sql_fetch($sql);

if ($type == "profile") {
    $file_name = $row['profileImage'];
    $file_field = "profileImage";
    $file_dir = '/files/userImage/';
}
...
if ($_FILES['bf_file']['name'][0] == "") {
    ...
    if (isset($_POST['file_name_del'][0]) && $_POST['file_name_del'][0]) {
        ...
        $sql = "update {$g5['member_table']} set $file_field = '' where id = '$u_id'";
        sql_query($sql);
    } else
