...
// 회원 정보를 얻는다.
function get_member_email($mb_email, $fields='*')
{
    global $g5;

	$sql = " select $fields from {$g5['member_table']} where email = TRIM('$mb_email') ";
    $row = sql_fetch($sql);
	$row[mb_id] = $row[id];
...
function member_delete($mb_id)
{
    ...
    $sql = " select * from {$g5['member_table']} where email= '".$mb_id."' ";
    $mb = sql_fetch($sql);
    ...
    $sql = " update {$g5['member_table']} set password = '', level = 1 where email = '{$mb_id}' ";
    sql_query($sql);
